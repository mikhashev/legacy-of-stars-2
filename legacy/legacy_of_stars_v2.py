...
class StarSystem:
    def __init__(self, name: str, distance: float):
        ...
        if self.has_civilization:
            ...
            human_age = 100  # Years since radio technology (Earth baseline)
            
            # 75% older than humanity, 25% younger
            if random.random() < 0.75:
                # Older civilization
                civ_age = human_age * random.uniform(1.5, 50)
            else:
                # Younger civilization
                civ_age = human_age * random.uniform(0.1, 0.9)
            
            # 10% chance of truly ancient civilization (>10x human age)
            if random.random() < 0.10:
                civ_age = human_age * random.uniform(10, 1000)
            
            self.civilization_age = civ_age
            
            # Map age to civilization stage (more realistic than random weights)
            self.civilization_stage = self._age_to_stage(civ_age)
            
            # 15% chance civilization is extinct (swan song opportunity)
            self.is_extinct = random.random() < 0.15
            if self.is_extinct:
                self.extinct_years_ago = random.randint(500, 5000)
                self.has_swan_song = random.random() < 0.8  # 80% left archive
                self.civilization_stage = None  # Extinct civs have no active stage
            
            # === Hidden Strategy System (Dark Forest Mechanics) ===
            # Player must deduce true nature from behavior
            # L = Listen only (never responds)
            # LB = Listen & Broadcast (friendly, active METI)
            # LR = Listen & Reply (defensive, only responds when contacted)
            # LA = Listen & Annihilate (predatory, attacks when contacted)
            # LBA = Listen, Broadcast & Annihilate (baiting trap)
            
            if not self.is_extinct:
                strategy_weights = {
                    "L": 10,    # Cautious isolationists
                    "LB": 30,   # Active seekers
                    "LR": 40,   # Reactive responders (most common)
                    "LA": 15,   # Predatory xenophobes
                    "LBA": 5    # Sophisticated predators (rarest, most dangerous)
                }
                ...
                # Deception capability (can LA/LBA pretend to be friendly?)
                # More advanced civilizations are better at deception
            ...
            # DEPRECATED: civilization_attitude (replaced by true_strategy)
            # Kept for backward compatibility but no longer used for behavior
        ...
        self.pending_attack = None  # Generation when LA/LBA attack arrives (if any)
    
    def _age_to_stage(self, age: float) -> CivilizationStage:
        """Map civilization age to Kardashev-based stage"""
        if age < 50:
