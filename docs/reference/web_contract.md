...
|---|---|---|---|
| `send_message` | `{system, text}` | a message was queued (`stats.messages_sent` grew) | `text` may be empty; costs 1 AP. Refused for a post-biological civilization without the matching technology. The reply, if any, is decided by the civilization as it will be when the signal *arrives* (`year + round(distance)`), so a target that is alive and talkative in every observation we have may still answer nothing; `message` never says which. |
...
| `systems[].messages_sent[]` | `{text, generation, arrival_gen, expected_reply_year, explanation_year, fate}` | our transmissions and what became of them. `generation` is when it was sent, `arrival_gen` when it reaches the target, `expected_reply_year` the in-game **year** an answer would arrive (`send year + 2 x distance`). `fate` is one of `"in_flight"` (a reply may still be coming), `"replied"` (one came), `"nobody"` (there was nobody who could answer, and we could already see that when we sent it) or `"unanswered"` (`expected_reply_year` has passed and nothing came). `explanation_year` is the year an observation explains the silence - the send year for `"nobody"`, the year the light of the target's death reaches Earth when it died before our signal landed - and is **null until that year has come**: the engine knows a target that will be gone on arrival, and the player must not, so neither `fate` nor `explanation_year` ever says so early. A save written before this field existed loads its messages as `"in_flight"` |
...
| `stats` | dict[str, int] | `messages_sent`, `responses_received`, `attacks_scheduled`, `attacks_survived`, `attacks_landed`, `info_attacks`, `swan_songs_found`, `systems_discovered`, `events_resolved`, `techs_researched`, `worlds_seeded`, `passive_detections`, plus the message outcomes `messages_replied`, `messages_nobody`, `messages_died_in_flight` and `messages_silent`. The last two are a **tally, not a per-message flag**: they are how the balance harness measures the receipt frame, and they cannot be matched back to a particular entry of `messages_sent[]`, which is what keeps them from leaking |
